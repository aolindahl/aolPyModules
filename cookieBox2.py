...
import random
import lmfit
from BurningDetectors_V3 import projector
...
class CookieBox:
    ...
    def getTimeAmplitudes(self, roi=None, verbose=False):
        return [tof.getTimeAmplitude(roi=roi) for tof in self._tofList]
    ...
    def getPhotonEnergy(self, energyShift=0):
        moments = np.array(self.getMoments(domain='Energy', roi=0))
        amplitudes = self.getIntensityDistribution(domain='Energy', roi=0)

        return (np.average(moments, axis=0, weights=amplitudes)
                + np.array([energyShift, 0]))


    def getIntensityDistribution(self, roi=None, domain='Time', verbose=False,
            dynamic=False, detFactors=[1]*16):
        if verbose:
        ...
        for amp, factor in zip(ampFunk(roi=roi, verbose=verbose), detFactors):
            if amp is None:
                intensity.append(np.nan)
            else:
                intensity.append(amp.sum() * factor)
    ...
    def randomizeAmplitudes(self, verbose=False):
        ...
        if verbose:
            for par in params.itervalues():
                print par
        factors = ( self.modelFunction(params, self.getAngles()) *
    ...
    def modelFunction(self, params, x, y=None, eps=None):
        A = params['A'].value
    ...
    def initialParams(self, yData=None):
        params = lmfit.Parameters()
        params.add('A', 10, min=0)
        params.add('beta', 2, min=-1, max=2)
        params.add('tilt', np.pi)
        params.add('linear', 0.5)
...
if __name__ == '__main__':
    ...
    for i, evt in enumerate(ds.events()):
        ...
        if i==0:
            ...
            timeScalesRoi0 = cb.getTimeScales_us(roi=0)
            timeScalesRoi1 = cb.getTimeScales_us(roi=1)
        ...
        if plot:
            ...
            timeDataRoi0 = cb.getTimeAmplitudes(roi=0)
            timeDataRoi1 = cb.getTimeAmplitudes(roi=1)
        ...
        amplitudes = cb.getIntensityDistribution(roi=0, domain=domain,
                verbose=True)
        params = cb.initialParams(amplitudes)
        #projTilt.append(params['tilt'].value)
        params['beta'].vary=False
        res = lmfit.minimize(cb.modelFunction, params, args=(angles, amplitudes),
                method='leastsq')
        ...
        if plot:
            ...
            fig2ax.lines[2].set_ydata(cb.modelFunction(params, anglesFit))
            #fig2ax.lines[3].set_ydata(amplitudes0.mean() * cb.modelFunction(randParams, anglesFit))
