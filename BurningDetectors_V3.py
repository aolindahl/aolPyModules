    ...
    def __init__(self):
        ...
        self._B = np.array(
                [ np.ones_like(self._phir)/16.,
                    ...
                    np.sin(2 * self._phir)/8. ]).T

    def setFitMask(self, fitMask=np.ones(16)):
        self._fitMask = fitMask.astype(bool)

        BMissing = np.diag( (~self._fitMask).astype(int) ).dot( self._B )
        self._BCorrected = np.linalg.inv(
                np.eye(3) - BMissing.T.dot( BMissing ) )

        self._fitMask = self._fitMask.astype(int)

    def solve(self, data, beta):
        BSignal = self._B.T.dot( self._fitMask * data )
        coef = self._BCorrected.dot(BSignal)
        print coef
        return self.params(coef, beta)

    def solveAllThere(self, data, beta):
        coef = self._B.T.dot(data)
        print coef
        return self.params(coef, beta)
